...
export const StudentTable = ( {students}: Props) => {
    return (
        <table className="w-full border border-gray-600 rounded-md overflow-hidden">
            <thead>
                <tr className="border-b border-gray-600 bg-gray-800 sm:text-left">
            ...
            <tbody>
               {students.map(item => (
                <tr key={item.id} className="text-gray-800 bg-gray-400 border-b border-gray-600 text-center">
                    <td className="p-3 flex items-center flex-col gap-2 sm:flex-row">
                        <img src={item.avatar} alt={item.name}  className="w-10 h-10 rounded-full mr-3"/>
                        <div>
                            <div className="font-bold sm:text-left">{item.name}</div>
                            <div>{item.email}</div>
                        </div>
                    </td>
                    <td className="p-3">
